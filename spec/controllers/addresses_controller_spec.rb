...
#   Non-Crud Based Methods
  describe "Get Search" do
    before :each do
      @address = create(:address)
    end

    it "returns an array of all states based on the first letter that matches" do
      @address = Address.create!(
          streetname:'5th Avenue', streetnumber:'West 23rd Street', building:'184',city:'Manhattan',state:'NY', zipcode:'10010')
      )
      get :index, {}, letter: 'N'
      expect(response).to include["NY","NJ","NH"]
    end

    it "returns all matching cities associated with a state" do
      @address = Address.create! valid_attributes
      get :search, id: @address
      expect(response).to include #all associated addresses?
    end

    it "returns all addresses associated with a zipcode" do
      get :index, #parameters?
      expect(response).to include[:addresses]
    end
    it "returns a suggestion of state based on the first letter input" do
      get :search, @address[:params => 'N']
      expect(response).to include[NY,NJ,NH]
    end
    it "returns a suggestion of city based on street name and number" do
        @address = build(:address, streetname: nil)
        @address.streetname = "Be"
        get :search, id: @address
        expect(response).to include["Bergen"]
    end
end
